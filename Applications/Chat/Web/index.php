...
<head>
    ...
    <script type="text/javascript">
        ...
        function onmessage(e) {
            ...
            switch (data['type']) {
                ...
                case 'login':
                    ...
                    show_say(data['client_id'], data['client_name'], data['client_name'] + ' 加入了聊天室', data['time']);
                    if (data['client_list']) {
                ...
                case 'say':
                    ...
                    show_say(data['from_client_id'], data['from_client_name'], data['content'], data['time']);
                    break;
                // 用户退出 更新用户列表
                case 'logout':
                    //{"type":"logout","client_id":xxx,"time":"xxx"}
                    show_say(data['from_client_id'], data['from_client_name'], data['from_client_name'] + ' 退出了', data['time']);
                    delete client_list[data['from_client_id']];
                    show_client_list(client_list);
            }
        }
        function sendMessage(to_client_id, to_client_name,content){
           var obj={"type":"say","to_client_id":  to_client_id , "to_client_name": to_client_name ,"content":content};
        ...
        function onSubmit() {
            ...
            sendMessage(to_client_id, to_client_name,content)
            input.value = "";
            input.focus();
        }

        // 刷新用户列表框
        function show_client_list(client_list) {
        ...
        // 发言
        function show_say(from_client_id, from_client_name, content, time) {
            //解析新浪微博图片
